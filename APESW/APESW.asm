...
;APESW.c,85 :: 		RD0_bit = 0;                           //Pone a cero despues de enviar todos los pulsos de exitacion.
	BCF         RD0_bit+0, BitPos(RD0_bit+0) 
;APESW.c,86 :: 		TMR1ON_bit=1;                          //Enciende el TMR1.
	BSF         TMR1ON_bit+0, BitPos(TMR1ON_bit+0) 
;APESW.c,87 :: 		TMR1L=0X00;                            //Limpia los bits menos significativos del TMR1.
	CLRF        TMR1L+0 
;APESW.c,88 :: 		TMR1H=0X00;                            //Limpia los bits mas significativos del TMR1.
	CLRF        TMR1H+0 
;APESW.c,89 :: 		}
L_Interrupt3:
;APESW.c,91 :: 		contp++;                                  //Aumenta el contador en una unidad.
	INFSNZ      _contp+0, 1 
	INCF        _contp+1, 1 
;APESW.c,92 :: 		TMR2IF_bit = 0;                           //Limpia la bandera de interrupcion de Timer2
	BCF         TMR2IF_bit+0, BitPos(TMR2IF_bit+0) 
;APESW.c,94 :: 		}
L_Interrupt0:
;APESW.c,97 :: 		if (INTCON.INT0IF == 1){                     //Verifica si ocurrio una interrupcion externa en INT0.
	BTFSS       INTCON+0, 1 
	GOTO        L_Interrupt4
;APESW.c,100 :: 		*(punT1) = TMR1L;                         //Carga el valor actual de TMR1L en los 8 bits menos significativos de la variable contT de tipo entero.
	MOVFF       _punT1+0, FSR1
	MOVFF       _punT1+1, FSR1H
	MOVF        TMR1L+0, 0 
	MOVWF       POSTINC1+0 
;APESW.c,101 :: 		*(punT1+1) = TMR1H;                       //Carga el valor actual de TMR1H en los 8 bits mas significativos de la variable  contT de tipo entero.
	MOVLW       1
	ADDWF       _punT1+0, 0 
	MOVWF       FSR1 
	MOVLW       0
	...
	MOVF        TMR1H+0, 0 
	MOVWF       POSTINC1+0 
;APESW.c,103 :: 		T2 = contT;                               //Carga el contenido actual de la variable contT en la variable T2.
	MOVF        _contT+0, 0 
	MOVWF       _T2+0 
	MOVF        _contT+1, 0 
	MOVWF       _T2+1 
...
;APESW.c,117 :: 		}
L_Interrupt5:
;APESW.c,119 :: 		if (DF1>0){                                     //Verifica si se habilito el inicio de deteccion de fase **
	MOVF        _DF1+0, 0 
	SUBLW       0
	BTFSC       STATUS+0, 0 
	GOTO        L_Interrupt11
...
;APESW.c,124 :: 		contTOF = T2;
	...
	MOVF        _T2+1, 0 
	MOVWF       _contTOF+1 
;APESW.c,125 :: 		RE1_bit = 0;
	BCF         RE1_bit+0, BitPos(RE1_bit+0) 
;APESW.c,126 :: 		DF1 = 0;
	CLRF        _DF1+0 
;APESW.c,127 :: 		TMR1ON_bit = 0;                          //Apaga el TMR1.
	BCF         TMR1ON_bit+0, BitPos(TMR1ON_bit+0) 
;APESW.c,128 :: 		contT = 0;                               //Limpia el contenido de la variable contT.
	CLRF        _contT+0 
	CLRF        _contT+1 
;APESW.c,129 :: 		}
L_Interrupt14:
;APESW.c,130 :: 		}
L_Interrupt11:
;APESW.c,133 :: 		T1 = contT;                                     //Actualiza T1 con el valor actual del contador contT.
	MOVF        _contT+0, 0 
	MOVWF       _T1+0 
	MOVF        _contT+1, 0 
	MOVWF       _T1+1 
