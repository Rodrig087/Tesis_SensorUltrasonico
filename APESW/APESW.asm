...
;APESW.c,74 :: 		if (TMR2IF_bit){                             //Verifica si ocurrio una interrupcion por desbordamiento del TMR2.
	...
	GOTO        L_Interrupt0
;APESW.c,76 :: 		if (contp<=39){                           //Controla el numero total de pulsos de exitacion del transductor ultrasonico. (43)
	MOVLW       0
	MOVWF       R0 
	MOVF        _contp+1, 0 
	SUBWF       R0, 0 
...
L__Interrupt29:
	BTFSS       STATUS+0, 0 
	GOTO        L_Interrupt1
;APESW.c,78 :: 		RD0_bit = ~RD0_bit;
	BTG         RD0_bit+0, BitPos(RD0_bit+0) 
;APESW.c,79 :: 		if (contp==22){
	MOVLW       0
	XORWF       _contp+1, 0 
	BTFSS       STATUS+0, 2 
	GOTO        L__Interrupt30
...
L__Interrupt30:
	BTFSS       STATUS+0, 2 
	GOTO        L_Interrupt2
;APESW.c,80 :: 		PR2 = 145;                          //Cambia el valor precargado en el TMR2 para producir una interrupcion cada 12,166666667us ~ 41,095KHz
	MOVLW       145
	MOVWF       PR2+0 
;APESW.c,81 :: 		}
L_Interrupt2:
;APESW.c,83 :: 		} else {
	GOTO        L_Interrupt3
L_Interrupt1:
;APESW.c,84 :: 		TMR2ON_bit=0;                          //Apaga el TMR2
...
;APESW.c,86 :: 		PR2 = 152;                             //Produce una interrupcion cada 12,5us
	MOVLW       152
	MOVWF       PR2+0 
...
;APESW.c,107 :: 		if (F1<=3){
	...
	BTFSS       STATUS+0, 0 
	GOTO        L_Interrupt5
;APESW.c,108 :: 		if (DT>(300-Tht)&&DT<(300+Tht)){  //Realiza una comparacion para verificar cuando se estabilice la primera fase de la senal
	MOVF        _ThT+0, 0 
	SUBLW       44
	MOVWF       R1 
	MOVLW       0
...
;APESW.c,109 :: 		F1++;
	INCF        _F1+0, 1 
;APESW.c,110 :: 		if (F1==3) {                       //Si 10 intervalos consecutivos cumplen con la condicion de estabilizacion, se empieza con el proceso de busqueda de cambio de fase
	MOVF        _F1+0, 0 
	XORLW       3
	BTFSS       STATUS+0, 2 
	GOTO        L_Interrupt9
;APESW.c,111 :: 		DF1 = T2;                       //Almacena el valor actual de la variable T2 para la referencia de inicio de deteccion de fase
	MOVF        _T2+0, 0 
	MOVWF       _DF1+0 
;APESW.c,112 :: 		RE1_bit = 1;
...
;APESW.c,118 :: 		}
L_Interrupt5:
;APESW.c,120 :: 		if (DF1>0){                                     //Verifica si se habilito el inicio de deteccion de fase **
	MOVF        _DF1+0, 0 
	SUBLW       0
	BTFSC       STATUS+0, 0 
	GOTO        L_Interrupt11
...
;APESW.c,123 :: 		DFT = ((F2*2)-1)*150;
	...
	MOVF        R3, 0 
	MOVWF       _DFT+0 
;APESW.c,124 :: 		if (DFT>(DF2-Tht)&&DFT<(DF2+Tht)){
	MOVF        _ThT+0, 0 
	SUBWF       R2, 0 
	MOVWF       R1 
	CLRF        R2 
...
L__Interrupt34:
	...
	GOTO        L_Interrupt14
L__Interrupt25:
;APESW.c,125 :: 		contTOF = T2;
	MOVF        _T2+0, 0 
	MOVWF       _contTOF+0 
	MOVF        _T2+1, 0 
	MOVWF       _contTOF+1 
;APESW.c,126 :: 		RE1_bit = 0;
	BCF         RE1_bit+0, BitPos(RE1_bit+0) 
;APESW.c,127 :: 		DF1 = 0;
	CLRF        _DF1+0 
;APESW.c,128 :: 		TMR1ON_bit = 0;                          //Apaga el TMR1.
	BCF         TMR1ON_bit+0, BitPos(TMR1ON_bit+0) 
;APESW.c,129 :: 		contT = 0;                             //Limpia el contenido de la variable contT.
	CLRF        _contT+0 
	CLRF        _contT+1 
;APESW.c,130 :: 		}
L_Interrupt14:
;APESW.c,131 :: 		}
L_Interrupt11:
;APESW.c,134 :: 		T1 = contT;                                     //Actualiza T1 con el valor actual del contador contT.
	MOVF        _contT+0, 0 
	MOVWF       _T1+0 
	MOVF        _contT+1, 0 
	MOVWF       _T1+1 
...
;APESW.c,194 :: 		PR2 = 152;
	MOVLW       152
	MOVWF       PR2+0 
;APESW.c,199 :: 		TRISD0_bit = 0;                             //Establece el pin D0 como salida
	BCF         TRISD0_bit+0, BitPos(TRISD0_bit+0) 
;APESW.c,200 :: 		TRISD1_bit = 0;                             //Establece el pin D1 como salida
	BCF         TRISD1_bit+0, BitPos(TRISD1_bit+0) 
;APESW.c,202 :: 		TRISE0_bit = 0;
	BCF         TRISE0_bit+0, BitPos(TRISE0_bit+0) 
;APESW.c,203 :: 		TRISE1_bit = 0;
	BCF         TRISE1_bit+0, BitPos(TRISE1_bit+0) 
;APESW.c,205 :: 		TRISB = 0x07;                               //Establece los pines B0, B1 y B2 como entradas
	MOVLW       7
	MOVWF       TRISB+0 
;APESW.c,207 :: 		}
L_end_Configuracion:
...
;APESW.c,210 :: 		void main() {
;APESW.c,212 :: 		Configuracion();
	CALL        _Configuracion+0, 0
;APESW.c,214 :: 		RD0_bit = 0;                                //Limpia el pin D0
	BCF         RD0_bit+0, BitPos(RD0_bit+0) 
;APESW.c,215 :: 		RD1_bit = 1;                                //Limpia el pin D1
	BSF         RD1_bit+0, BitPos(RD1_bit+0) 
;APESW.c,216 :: 		RE1_bit = 0;
	BCF         RE1_bit+0, BitPos(RE1_bit+0) 
...
;APESW.c,273 :: 		}
	GOTO        L_main21
L_main22:
;APESW.c,275 :: 		FloatToStr(TOF, txt1);
	MOVF        _TOF+0, 0 
