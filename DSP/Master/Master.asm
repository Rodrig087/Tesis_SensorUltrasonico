...
;Master.c,107 :: 		Ptcn[0]=Hdr;
	MOVLW       238
	MOVWF       _Ptcn+0 
;Master.c,108 :: 		Ptcn[1]=Tp;
	MOVLW       1
	MOVWF       _Ptcn+1 
;Master.c,109 :: 		Ptcn[2]=Id;
	MOVLW       7
	MOVWF       _Ptcn+2 
;Master.c,110 :: 		Ptcn[3]=End;
	MOVLW       255
	MOVWF       _Ptcn+3 
;Master.c,112 :: 		Bb=0;
	CLRF        _Bb+0 
;Master.c,113 :: 		Dst=0;
	CLRF        _Dst+0 
	CLRF        _Dst+1 
;Master.c,115 :: 		while (1){
L_main12:
;Master.c,117 :: 		if ((RA0_bit==1)&&(Bb==0)){
...
;Master.c,118 :: 		Bb = 1;
	MOVLW       1
	MOVWF       _Bb+0 
;Master.c,119 :: 		RC5_bit = 1;                                         //Establece el Max485 en modo de escritura
	BSF         RC5_bit+0, BitPos(RC5_bit+0) 
;Master.c,120 :: 		for (ip=0;ip<Psize;ip++){
	CLRF        _ip+0 
L_main17:
	MOVLW       128
	XORWF       _ip+0, 0 
	...
	BTFSC       STATUS+0, 0 
	GOTO        L_main18
;Master.c,121 :: 		UART1_WRITE(Ptcn[ip]);                          //Manda por Uart la trama de peticion
	MOVLW       _Ptcn+0
	MOVWF       FSR0 
...
;Master.c,123 :: 		Dst = 0;
	CLRF        _Dst+0 
	CLRF        _Dst+1 
;Master.c,124 :: 		while(UART_Tx_Idle()==0);                            //Espera hasta que se haya terminado de enviar todo el dato por UART antes de continuar
L_main20:
	...
	GOTO        L_main20
L_main21:
;Master.c,125 :: 		RC5_bit = 0;                                         //Establece el Max485 en modo de lectura;
	BCF         RC5_bit+0, BitPos(RC5_bit+0) 
;Master.c,126 :: 		} else if (RA0_bit==0){
	GOTO        L_main22
L_main16:
...
L_main23:
L_main22:
;Master.c,130 :: 		if (BanP==1){
	MOVF        _BanP+0, 0 
	XORLW       1
	BTFSS       STATUS+0, 2 
	GOTO        L_main24
;Master.c,131 :: 		if ((Rspt[0]==Hdr)&&(Rspt[Rsize-1]==End)){
	MOVF        _Rspt+0, 0 
	XORLW       238
	BTFSS       STATUS+0, 2 
	GOTO        L_main27
	...
	GOTO        L_main27
L__main46:
;Master.c,132 :: 		if ((Rspt[1]==TP)&&(Rspt[2]==Id)){                //Verifica el identificador de tipo de sensor y el identificador de esclavo
	MOVF        _Rspt+1, 0 
	XORLW       1
	BTFSS       STATUS+0, 2 
	GOTO        L_main30
	...
	GOTO        L_main30
L__main45:
;Master.c,134 :: 		for (irr=3;irr<5;irr++){
	MOVLW       3
	MOVWF       _irr+0 
L_main31:
	MOVLW       128
	...
	BTFSC       STATUS+0, 0 
	GOTO        L_main32
;Master.c,135 :: 		*(ptrDst+(irr-3)) = Rspt[irr];               //Asigna a TT2 los datos tomados de la trama de peticion
	MOVLW       3
	SUBWF       _irr+0, 0 
	MOVWF       R0 
	MOVLW       0
...
;Master.c,136 :: 		}
	GOTO        L_main31
L_main32:
;Master.c,137 :: 		for (irr=0;irr<(Rsize-1);irr++){
	CLRF        _irr+0 
L_main34:
	MOVLW       128
	XORWF       _irr+0, 0 
	...
	BTFSC       STATUS+0, 0 
	GOTO        L_main35
;Master.c,138 :: 		Rspt[irr]=0;;                            //Limpia los bits de datos de la trama de respuesta
	MOVLW       _Rspt+0
	MOVWF       FSR1 
	MOVLW       hi_addr(_Rspt+0)
	MOVWF       FSR1H 
...
;Master.c,143 :: 		} else {
	GOTO        L_main37
L_main27:
;Master.c,145 :: 		for (irr=0;irr<(Rsize-1);irr++){
	CLRF        _irr+0 
L_main38:
	MOVLW       128
	XORWF       _irr+0, 0 
	...
	BTFSC       STATUS+0, 0 
	GOTO        L_main39
;Master.c,146 :: 		Rspt[irr]=0;;                            //Limpia los bits de datos de la trama de respuesta
	MOVLW       _Rspt+0
	MOVWF       FSR1 
	MOVLW       hi_addr(_Rspt+0)
	MOVWF       FSR1H 
...
;Master.c,151 :: 		}
L_main24:
;Master.c,154 :: 		IntToStr(Dst,txt1);
	MOVF        _Dst+0, 0 
	...
	CALL        _IntToStr+0, 0
;Master.c,156 :: 		Lcd_Out(1,1,"Distancia:");
	MOVLW       1
