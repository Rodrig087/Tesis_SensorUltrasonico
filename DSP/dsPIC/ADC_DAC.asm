...
;ADC_DAC.c,219 :: 		chIDst = (unsigned char *) & IDst;       //Asocia el valor calculado de Dst al puntero chDst
	...
	MOV	W0, _chIDst
;ADC_DAC.c,221 :: 		for (ir=4;ir>=3;ir--){
	MOV	#lo_addr(_ir), W1
	MOV.B	#4, W0
	MOV.B	W0, [W1]
L_Distancia18:
...
;ADC_DAC.c,274 :: 		RB2_bit = ~RB2_bit;                      //Conmuta el valor del pin RB14
	BTG	RB2_bit, BitPos(RB2_bit+0)
;ADC_DAC.c,275 :: 		}else {
	GOTO	L_Timer2Interrupt36
L_Timer2Interrupt35:
;ADC_DAC.c,276 :: 		RB2_bit = 0;                            //Pone a cero despues de enviar todos los pulsos de exitacion.
	BCLR	RB2_bit, BitPos(RB2_bit+0)
...
L_main40:
;ADC_DAC.c,390 :: 		Distancia();                                       //Realiza un calculo de distancia
	CALL	_Distancia
;ADC_DAC.c,393 :: 		for (ir=0;ir<Rsize;ir++){
	MOV	#lo_addr(_ir), W1
	CLR	W0
	MOV.B	W0, [W1]
L_main42:
	...
	GOTO	L_main43
L__main90:
;ADC_DAC.c,394 :: 		UART1_Write(Rspt[ir]);                         //Envia la trama de respuesta
	MOV	#lo_addr(_ir), W0
	ZE	[W0], W1
	MOV	#lo_addr(_Rspt), W0
	ADD	W0, W1, W0
...
;ADC_DAC.c,395 :: 		}
	GOTO	L_main42
L_main43:
;ADC_DAC.c,396 :: 		UART1_Write(0x0D);
	MOV	#13, W10
	CALL	_UART1_Write
...
L__main91:
	GOTO	L_main45
L_main46:
;ADC_DAC.c,398 :: 		for (ipp=3;ipp<5;ipp++){
	MOV	#lo_addr(_ipp), W1
	MOV.B	#3, W0
	MOV.B	W0, [W1]
L_main47:
	...
	GOTO	L_main48
L__main92:
;ADC_DAC.c,399 :: 		Rspt[ipp]=0;;                                  //Limpia los bits de datos de la trama de respuesta
	MOV	#lo_addr(_ipp), W0
	ZE	[W0], W1
	MOV	#lo_addr(_Rspt), W0
	ADD	W0, W1, W1
