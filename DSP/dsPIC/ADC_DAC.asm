...
;ADC_DAC.c,71 :: 		Ow_Reset(&PORTA, 0);                        //Onewire reset signal
	PUSH	W10
	PUSH	W11
	PUSH	W12
	CLR	W11
	...
	CALL	_Ow_Reset
;ADC_DAC.c,72 :: 		Ow_Write(&PORTA, 0, 0xCC);                  //Issue command SKIP_ROM
	MOV.B	#204, W12
	CLR	W11
	MOV	#lo_addr(PORTA), W10
	CALL	_Ow_Write
;ADC_DAC.c,73 :: 		Ow_Write(&PORTA, 0, 0x44);                  //Issue command CONVERT_T
	MOV.B	#68, W12
	CLR	W11
	MOV	#lo_addr(PORTA), W10
	CALL	_Ow_Write
...
L_Velocidad0:
	...
	BRA NZ	L_Velocidad0
	NOP
;ADC_DAC.c,76 :: 		Ow_Reset(&PORTA, 0);
	CLR	W11
	MOV	#lo_addr(PORTA), W10
	CALL	_Ow_Reset
;ADC_DAC.c,77 :: 		Ow_Write(&PORTA, 0, 0xCC);                  //Issue command SKIP_ROM
	MOV.B	#204, W12
	CLR	W11
	MOV	#lo_addr(PORTA), W10
	CALL	_Ow_Write
;ADC_DAC.c,78 :: 		Ow_Write(&PORTA, 0, 0xBE);                  //Issue command READ_SCRATCHPAD
	MOV.B	#190, W12
	CLR	W11
	MOV	#lo_addr(PORTA), W10
	CALL	_Ow_Write
...
L_Velocidad2:
	...
	BRA NZ	L_Velocidad2
	NOP
;ADC_DAC.c,81 :: 		Temp =  Ow_Read(&PORTA, 0);
	CLR	W11
	MOV	#lo_addr(PORTA), W10
	CALL	_Ow_Read
; Temp start address is: 10 (W5)
	ZE	W0, W5
;ADC_DAC.c,82 :: 		Temp = (Ow_Read(&PORTA, 0) << 8) + Temp;
	CLR	W11
	MOV	#lo_addr(PORTA), W10
	CALL	_Ow_Read
	ZE	W0, W0
...
;ADC_DAC.c,101 :: 		contp = 0;                                               //Limpia la variable del contador de pulsos
	CLR	W0
	MOV	W0, _contp
;ADC_DAC.c,102 :: 		RB2_bit = 0;                                            //Limpia el pin que produce los pulsos de exitacion del transductor
	BCLR	RB2_bit, BitPos(RB2_bit+0)
;ADC_DAC.c,104 :: 		T1CON.TON = 0;                                           //Apaga el TMR1
	BCLR	T1CON, #15
;ADC_DAC.c,105 :: 		IEC0.T1IE = 0;                                           //Desabilita la interrupcion por desborde del TMR1
	BCLR	IEC0, #3
;ADC_DAC.c,107 :: 		TMR2 = 0;                                                //Encera el TMR2
	CLR	TMR2
;ADC_DAC.c,108 :: 		IEC0.T2IE = 1;                                           //Habilita la interrupcion por desborde del TMR2
	BSET	IEC0, #7
;ADC_DAC.c,109 :: 		T2CON.TON = 1;                                           //Enciende el TMR2
	BSET	T2CON, #15
;ADC_DAC.c,111 :: 		i = 0;                                                   //Limpia las variables asociadas al almacenamiento de la señal muestreada
	CLR	W0
	MOV	W0, _i
;ADC_DAC.c,112 :: 		j = 0;
	CLR	W0
	MOV	W0, _j
;ADC_DAC.c,116 :: 		while(bm!=1);                                            //Espera hasta que haya terminado de enviar y recibir todas las muestras
L_Pulse5:
	MOV	#lo_addr(_bm), W0
	MOV.B	[W0], W0
	CP.B	W0, #1
...
;ADC_DAC.c,119 :: 		if (bm==1){
	...
	GOTO	L_Pulse7
L__Pulse50:
;ADC_DAC.c,122 :: 		Mmax = Vector_Max(M, nm, &MIndexMax);
	...
	SUB	#6, W15
	MOV	W0, _Mmax
;ADC_DAC.c,123 :: 		Mmin = Vector_Min(M, nm, &MIndexMin);
	...
	SUB	#6, W15
	MOV	W0, _Mmin
;ADC_DAC.c,124 :: 		Mmed = Mmax-((Mmax-Mmin)/2);
...
;ADC_DAC.c,130 :: 		if (M[k]<Mmed){
	...
	GOTO	L_Pulse11
L__Pulse52:
;ADC_DAC.c,131 :: 		value = (M[k]+((Mmed-M[k])*2))-(Mmed);
	MOV	_k, W0
	SL	W0, #1, W1
	MOV	#lo_addr(_M), W0
	ADD	W0, W1, W1
...
;ADC_DAC.c,132 :: 		}
L_Pulse11:
;ADC_DAC.c,135 :: 		x0 = (float)(value);                                 //Adquisición de una muestra de 10 bits en, x[0].
	MOV	_value, W0
	CLR	W1
	CALL	__Long2Float
	MOV	W0, _x0
	MOV	W1, _x0+2
;ADC_DAC.c,136 :: 		y0 = ((x0+x2)*ca1)+(x1*ca2)-(y1*cb2)-(y2*cb3);       //Implementación de la ecuación en diferencias
	MOV	_x2, W2
	MOV	_x2+2, W3
	CALL	__AddSub_FP
	MOV	#58496, W2
	...
	MOV	W0, _y0
	MOV	W1, _y0+2
;ADC_DAC.c,138 :: 		y2 = y1;                                             //Corrimiento de los valores x(n), y y(n).
	MOV	_y1, W2
	MOV	_y1+2, W3
	MOV	W2, _y2
	MOV	W3, _y2+2
...
;ADC_DAC.c,141 :: 		x1 = x0;
	...
	MOV	W2, _x1
	MOV	W3, _x1+2
;ADC_DAC.c,143 :: 		YY = (unsigned int)(y0);                             //Reconstrucción de la señal: y en 10 bits.
...
;ADC_DAC.c,146 :: 		}
	GOTO	L_Pulse8
L_Pulse9:
;ADC_DAC.c,148 :: 		bm = 2;                                                  //Cambia el estado de la bandera bm para dar paso al cálculo del pmax y TOF
	MOV	#lo_addr(_bm), W1
	MOV.B	#2, W0
	MOV.B	W0, [W1]
;ADC_DAC.c,150 :: 		}
L_Pulse7:
;ADC_DAC.c,153 :: 		if (bm==2){
	MOV	#lo_addr(_bm), W0
	MOV.B	[W0], W0
	CP.B	W0, #2
	BRA Z	L__Pulse53
	GOTO	L_Pulse12
L__Pulse53:
;ADC_DAC.c,155 :: 		yy1 = Vector_Max(M, nm, &maxIndex);                         //Encuentra el valor maximo del vector R
	...
	SUB	#6, W15
	MOV	W0, _yy1
;ADC_DAC.c,156 :: 		i1b = maxIndex;                                              //Asigna el subindice del valor maximo a la variable i1a
	MOV	_maxIndex, W0
	MOV	W0, _i1b
;ADC_DAC.c,157 :: 		i1a = 0;
	CLR	W0
	MOV	W0, _i1a
;ADC_DAC.c,159 :: 		while (M[i1a]<yy1){
L_Pulse13:
	MOV	_i1a, W0
	SL	W0, #1, W1
	MOV	#lo_addr(_M), W0
...
;ADC_DAC.c,161 :: 		}
	GOTO	L_Pulse13
L_Pulse14:
;ADC_DAC.c,163 :: 		i1 = i1a+((i1b-i1a)/2);
	MOV	_i1b, W1
	MOV	#lo_addr(_i1a), W0
	SUB	W1, [W0], W0
	LSR	W0, #1, W1
	...
	ADD	W1, [W0], W1
	MOV	W1, _i1
;ADC_DAC.c,164 :: 		i0 = i1 - dix;
	SUB	W1, #15, W0
	MOV	W0, _i0
;ADC_DAC.c,165 :: 		i2 = i1 + dix;
	ADD	W1, #15, W3
	MOV	W3, _i2
;ADC_DAC.c,167 :: 		yy0 = M[i0];
	SL	W0, #1, W1
	MOV	#lo_addr(_M), W0
	ADD	W0, W1, W0
	MOV	[W0], W2
	MOV	W2, _yy0
;ADC_DAC.c,168 :: 		yy2 = M[i2];
	SL	W3, #1, W1
	MOV	#lo_addr(_M), W0
	ADD	W0, W1, W0
	MOV	[W0], W0
...
;ADC_DAC.c,172 :: 		yf2 = (float)(yy2);
	...
	MOV	W1, _yf2+2
	POP.D	W0
;ADC_DAC.c,174 :: 		nx = (yf0-yf2)/(2.0*(yf0-(2.0*yf1)+yf2));                   //Factor de ajuste determinado por interpolacion parabolica
	MOV	[W14+8], W0
	MOV	[W14+10], W1
	PUSH.D	W2
	MOV	[W14+0], W2
...
;ADC_DAC.c,176 :: 		tmax = i1*tx;
	...
	MOV	W0, _tmax
	MOV	W1, _tmax+2
;ADC_DAC.c,178 :: 		T2 = tmax+dx;
	MOV	_dx, W2
	MOV	_dx+2, W3
	CALL	__AddSub_FP
	MOV	W0, _T2
	MOV	W1, _T2+2
;ADC_DAC.c,179 :: 		imax = (unsigned int)(T2/tx);
	MOV	#0, W2
	MOV	#16544, W3
	CALL	__Div_FP
	CALL	__Float2Longint
	MOV	W0, _imax
;ADC_DAC.c,181 :: 		IEC0.T1IE = 1;                                           //Habilita la interrupcion por desborde del TMR1 para dar inicio al muestreo del ADC
	BSET	IEC0, #3
;ADC_DAC.c,182 :: 		TMR1 = 0;                                                //Encera el TMR1
	CLR	TMR1
;ADC_DAC.c,183 :: 		T1IF_bit = 0;                                            //Limpia la bandera de interrupcion por desbordamiento del TMR1
	BCLR	T1IF_bit, BitPos(T1IF_bit+0)
;ADC_DAC.c,184 :: 		T1CON.TON = 1;                                           //Enciende el TMR1
	BSET	T1CON, #15
;ADC_DAC.c,185 :: 		bm = 3;
	MOV	#lo_addr(_bm), W1
	MOV.B	#3, W0
...
;ADC_DAC.c,194 :: 		void Timer1Interrupt() iv IVT_ADDR_T1INTERRUPT{
;ADC_DAC.c,195 :: 		RB3_bit = ~RB3_bit;
	BTG	RB3_bit, BitPos(RB3_bit+0)
;ADC_DAC.c,196 :: 		if (bm==0){                                   //Cuando la bandera bm=0, la interrupcion por TMR1 es utilizada para el muestreo de la señal de entrada
	MOV	#lo_addr(_bm), W0
	MOV.B	[W0], W0
	CP.B	W0, #0
	BRA Z	L__Timer1Interrupt57
	GOTO	L_Timer1Interrupt17
L__Timer1Interrupt57:
;ADC_DAC.c,197 :: 		SAMP_bit = 0;                              //Limpia el bit SAMP para iniciar la conversion del ADC
	BCLR	SAMP_bit, BitPos(SAMP_bit+0)
;ADC_DAC.c,198 :: 		while (!AD1CON1bits.DONE);                 //Espera hasta que se complete la conversion
L_Timer1Interrupt18:
	BTSC	AD1CON1bits, #0
	GOTO	L_Timer1Interrupt19
	GOTO	L_Timer1Interrupt18
L_Timer1Interrupt19:
;ADC_DAC.c,199 :: 		if (i<nm){
	MOV	_i, W1
	MOV	#350, W0
	CP	W1, W0
	BRA LTU	L__Timer1Interrupt58
	GOTO	L_Timer1Interrupt20
L__Timer1Interrupt58:
;ADC_DAC.c,200 :: 		M[i] = ADC1BUF0;                        //Almacena el valor actual de la conversion del ADC en el vector M
	MOV	_i, W0
	SL	W0, #1, W1
	MOV	#lo_addr(_M), W0
	ADD	W0, W1, W1
	MOV	ADC1BUF0, WREG
	MOV	W0, [W1]
;ADC_DAC.c,201 :: 		i++;                                    //Aumenta en 1 el subindice del vector de Muestras
	MOV	#1, W1
	MOV	#lo_addr(_i), W0
	ADD	W1, [W0], [W0]
;ADC_DAC.c,202 :: 		} else {
...
;ADC_DAC.c,207 :: 		}
L_Timer1Interrupt17:
;ADC_DAC.c,209 :: 		if (bm==3) {                                  //Cuando la bandera bm=3, la interrupcion por TMR1 es utilizada para la reconstruccion de la señal mediante el DAC
	MOV	#lo_addr(_bm), W0
	MOV.B	[W0], W0
	CP.B	W0, #3
	BRA Z	L__Timer1Interrupt59
...
;ADC_DAC.c,210 :: 		if (j<nm){
	...
	MOV	#350, W0
	CP	W1, W0
	BRA LTU	L__Timer1Interrupt60
	GOTO	L_Timer1Interrupt23
L__Timer1Interrupt60:
;ADC_DAC.c,212 :: 		j++;
	MOV	#1, W1
	MOV	#lo_addr(_j), W0
	ADD	W1, [W0], [W0]
...
;ADC_DAC.c,223 :: 		if (contp<22){                                //Controla el numero total de pulsos de exitacion del transductor ultrasonico. (
	MOV	_contp, W0
	CP	W0, #22
	BRA LTU	L__Timer2Interrupt62
	GOTO	L_Timer2Interrupt25
L__Timer2Interrupt62:
;ADC_DAC.c,224 :: 		if (contp==11){
	MOV	_contp, W0
	CP	W0, #11
	BRA Z	L__Timer2Interrupt63
	GOTO	L_Timer2Interrupt26
L__Timer2Interrupt63:
;ADC_DAC.c,225 :: 		BT2 = ~BT2;
	MOV	#lo_addr(_bT2), W0
	COM.B	[W0], [W0]
;ADC_DAC.c,226 :: 		}
L_Timer2Interrupt26:
;ADC_DAC.c,227 :: 		BT2 = ~BT2;
	MOV	#lo_addr(_bT2), W0
	MOV.B	[W0], W0
	COM.B	W0, W1
	MOV	#lo_addr(_bT2), W0
	MOV.B	W1, [W0]
;ADC_DAC.c,228 :: 		RB2_bit = BT2;                    //Conmuta el valor del pin RB14
	BTSS	W1, #0
	BCLR	RB2_bit, BitPos(RB2_bit+0)
	BTSC	W1, #0
...
;ADC_DAC.c,232 :: 		if (contp==110){
	...
	CP	W1, [W0]
	BRA Z	L__Timer2Interrupt64
	GOTO	L_Timer2Interrupt28
L__Timer2Interrupt64:
...
;ADC_DAC.c,260 :: 		TRISB = 0xFF00;                             //TRISB = 11111111 00000000
	MOV	#65280, W0
	MOV	WREG, TRISB
