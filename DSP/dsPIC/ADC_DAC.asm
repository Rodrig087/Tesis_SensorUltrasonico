...
;ADC_DAC.c,220 :: 		if ((T2b-T2a)<=T2umb){             //Verifica si el T2 actual esta dentro de un umbral pre-establecido
	...
	CP0	W0
	CLR.B	W0
	BRA GT	L__Calcular122
	INC.B	W0
L__Calcular122:
	CP0.B	W0
	BRA NZ	L__Calcular123
	GOTO	L_Calcular17
L__Calcular123:
...
;ADC_DAC.c,312 :: 		if ((T2b-T2a)<=T2umb){             //Verifica si el T2 actual esta dentro de un umbral pre-establecido
	...
	CP0	W0
	CLR.B	W0
	BRA GT	L__Calibracion142
	INC.B	W0
L__Calibracion142:
	POP	W10
	CP0.B	W0
	BRA NZ	L__Calibracion143
...
L_Timer1Interrupt64:
;ADC_DAC.c,371 :: 		bm = 1;                                    //Cambia el valor de la bandera bm para terminar con el muestreo y dar comienzo al procesamiento de la señal
	MOV	#lo_addr(_bm), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
...
;ADC_DAC.c,417 :: 		AD1CON1.SIMSAM = 0;                         //0 -> Muestrea múltiples canales individualmente en secuencia
	BCLR	AD1CON1, #3
...
;ADC_DAC.c,424 :: 		AD1CON2.CSCNA = 0;                          //No escanea las entradas de CH0 durante la Muestra A
	BCLR	AD1CON2, #10
;ADC_DAC.c,425 :: 		AD1CON2.BUFM = 0;                           //Bit de selección del modo de relleno del búfer, 0 -> Siempre comienza a llenar el buffer desde el principio
	BCLR	AD1CON2, #1
;ADC_DAC.c,426 :: 		AD1CON2.ALTS = 0x00;                        //Utiliza siempre la selección de entrada de canal para la muestra A
	BCLR	AD1CON2, #0
...
;ADC_DAC.c,480 :: 		Id = (PORTB&0xFF00)>>8;                                  //Lee el Id de esclavo establecido por el dipswitch
	...
	LSR	W0, #8, W2
	MOV	W2, _Id
;ADC_DAC.c,481 :: 		Alt = 300;                                               //Establece la altura de instalacion del sensor en 300 mm
	MOV	#300, W0
	MOV	W0, _Alt
;ADC_DAC.c,482 :: 		T2adj = 477.0;                                           //Factor de calibracion de T2: Con Temp=20 y Vsnd=343.2, reduce la medida 1mm por cada 3 unidades que se aumente a este factor
	MOV	#32768, W0
	MOV	#17390, W1
	MOV	W0, _T2adj
	MOV	W1, _T2adj+2
;ADC_DAC.c,484 :: 		chDP = &DatoPtcn;                                        //Asocia el valor de DatoPtcn al puntero chDP
...
;ADC_DAC.c,485 :: 		ip=0;
	...
	CLR	W0
	MOV.B	W0, [W1]
;ADC_DAC.c,487 :: 		Rspt[0] = Hdr;                                           //Se rellena el primer byte de la trama de respuesta con el delimitador de inicio de trama
	MOV	#lo_addr(_Rspt), W1
	MOV.B	#58, W0
	MOV.B	W0, [W1]
;ADC_DAC.c,488 :: 		Rspt[1] = Id;                                            //Se rellena el segundo byte de la trama de repuesta con el Id del tipo de sensor
	MOV	#lo_addr(_Rspt+1), W0
	MOV.B	W2, [W0]
;ADC_DAC.c,489 :: 		Rspt[Rsize-1] = End;                                     //Se rellena el ultimo byte de la trama de repuesta con el delimitador de final de trama
	MOV	#lo_addr(_Rspt+5), W1
	MOV.B	#13, W0
	MOV.B	W0, [W1]
;ADC_DAC.c,491 :: 		num=0x30;
	MOV	#lo_addr(_num), W1
	MOV.B	#48, W0
	MOV.B	W0, [W1]
;ADC_DAC.c,493 :: 		while(1){
L_main71:
;ADC_DAC.c,496 :: 		if (BanP==1){                                   //Verifica si se realizo una peticion
	...
	GOTO	L_main73
L__main160:
;ADC_DAC.c,497 :: 		if ((Ptcn[1]==Id)&&(Ptcn[Psize-1]==End)){    //Verifica el identificador de esclavo y el byte de final de trama
...
L__main162:
L__main109:
;ADC_DAC.c,499 :: 		Fcn = Ptcn[2];                            //Almacena el tipo de funcion requerida
	MOV	#lo_addr(_Fcn), W1
	MOV	#lo_addr(_Ptcn+2), W0
	MOV.B	[W0], [W1]
;ADC_DAC.c,501 :: 		if (Fcn==0x01){                           //01: Lee el registro principal (Distancia)
	MOV	#lo_addr(_Ptcn+2), W0
	MOV.B	[W0], W0
	CP.B	W0, #1
	BRA Z	L__main163
	GOTO	L_main77
L__main163:
;ADC_DAC.c,502 :: 		Calcular();                            //Realiza una secuencia de calculo
	CALL	_Calcular
;ADC_DAC.c,503 :: 		Responder(0x01);                       //Envia la trama de repuesta con el valor del registro 0x01
	MOV	#1, W10
	CALL	_Responder
;ADC_DAC.c,504 :: 		}
L_main77:
;ADC_DAC.c,505 :: 		if (Fcn==0x02){                           //02: Lee el registro especicfico (01:Distancia, 02:Caudal, 03:Temperatura)
	MOV	#lo_addr(_Fcn), W0
	MOV.B	[W0], W0
	CP.B	W0, #2
	BRA Z	L__main164
	GOTO	L_main78
L__main164:
;ADC_DAC.c,506 :: 		Calcular();                            //Realiza una secuencia de calculo
	CALL	_Calcular
;ADC_DAC.c,507 :: 		*chDP = Ptcn[4];                       //Almacena el byte 4 de la trama de peticion en el LSB de la variable DatoPtcn
	MOV	#lo_addr(_Ptcn+4), W1
	MOV	_chDP, W0
	MOV.B	[W1], [W0]
;ADC_DAC.c,508 :: 		*(chDP+1) = Ptcn[3];                   //Almacena el byte 3 de la trama de peticion en el MSB de la variable DatoPtcn
	MOV	_chDP, W0
	ADD	W0, #1, W1
	MOV	#lo_addr(_Ptcn+3), W0
	MOV.B	[W0], [W1]
;ADC_DAC.c,509 :: 		Responder(DatoPtcn);                   //Envia la trama de repuesta con el valor del registro requerido
	MOV	_DatoPtcn, W10
	CALL	_Responder
;ADC_DAC.c,510 :: 		}
L_main78:
;ADC_DAC.c,511 :: 		if (Fcn==0x03){                           //03: Establece la altura de instalacion del sensor
	MOV	#lo_addr(_Fcn), W0
	MOV.B	[W0], W0
	CP.B	W0, #3
	BRA Z	L__main165
	GOTO	L_main79
L__main165:
;ADC_DAC.c,512 :: 		*chDP = Ptcn[4];                       //Almacena el byte 4 de la trama de peticion en el LSB de la variable DatoPtcn
	MOV	#lo_addr(_Ptcn+4), W1
	MOV	_chDP, W0
	MOV.B	[W1], [W0]
;ADC_DAC.c,513 :: 		*(chDP+1) = Ptcn[3];                   //Almacena el byte 3 de la trama de peticion en el MSB de la variable DatoPtcn
	MOV	_chDP, W0
	ADD	W0, #1, W1
	MOV	#lo_addr(_Ptcn+3), W0
	MOV.B	[W0], [W1]
;ADC_DAC.c,514 :: 		Alt = DatoPtcn;
	MOV	_DatoPtcn, W0
	MOV	W0, _Alt
;ADC_DAC.c,515 :: 		}
L_main79:
;ADC_DAC.c,516 :: 		if (Fcn==0x04){                           //04: Calibra el sensor en funcion del dato de la distancia real del sensor a la superficie del liquido
	MOV	#lo_addr(_Fcn), W0
	MOV.B	[W0], W0
	CP.B	W0, #4
	BRA Z	L__main166
...
;ADC_DAC.c,518 :: 		*(chDP+1) = Ptcn[3];                   //Almacena el byte 3 de la trama de peticion en el MSB de la variable DatoPtcn
	...
	MOV	#lo_addr(_Ptcn+3), W0
	MOV.B	[W0], [W1]
;ADC_DAC.c,519 :: 		Calibracion(DatoPtcn);                 //Realiza un proceso de calibracion para calcular el valor de la variable T2adj
	MOV	_DatoPtcn, W10
	CALL	_Calibracion
;ADC_DAC.c,520 :: 		}
L_main80:
;ADC_DAC.c,521 :: 		if (Fcn==0x05){                           //Test
	MOV	#lo_addr(_Fcn), W0
	MOV.B	[W0], W0
	CP.B	W0, #5
	BRA Z	L__main167
	GOTO	L_main81
L__main167:
;ADC_DAC.c,522 :: 		Rspt[2]=Ptcn[2];                       //Rellena el byte 2 con el tipo de funcion de la trama de peticion
	MOV	#lo_addr(_Rspt+2), W1
	MOV	#lo_addr(_Ptcn+2), W0
	MOV.B	[W0], [W1]
;ADC_DAC.c,523 :: 		Rspt[3]=Ptcn[3];
	MOV	#lo_addr(_Rspt+3), W1
	MOV	#lo_addr(_Ptcn+3), W0
	MOV.B	[W0], [W1]
;ADC_DAC.c,524 :: 		Rspt[4]=Ptcn[4];
	MOV	#lo_addr(_Rspt+4), W1
	MOV	#lo_addr(_Ptcn+4), W0
	MOV.B	[W0], [W1]
;ADC_DAC.c,525 :: 		RB5_bit = 1;                           //Establece el Max485 en modo de escritura
	BSET	RB5_bit, BitPos(RB5_bit+0)
;ADC_DAC.c,526 :: 		for (ir=0;ir<Rsize;ir++){
	MOV	#lo_addr(_ir), W1
	CLR	W0
	MOV.B	W0, [W1]
L_main82:
	...
	GOTO	L_main83
L__main168:
;ADC_DAC.c,527 :: 		UART1_Write(Rspt[ir]);             //Envia la trama de respuesta
	MOV	#lo_addr(_ir), W0
	ZE	[W0], W1
	MOV	#lo_addr(_Rspt), W0
	ADD	W0, W1, W0
...
;ADC_DAC.c,528 :: 		}
	GOTO	L_main82
L_main83:
;ADC_DAC.c,529 :: 		while(UART1_Tx_Idle()==0);             //Espera hasta que se haya terminado de enviar todo el dato por UART antes de continuar
L_main85:
	CALL	_UART1_Tx_Idle
	CP	W0, #0
	BRA Z	L__main169
...
L__main169:
	GOTO	L_main85
L_main86:
;ADC_DAC.c,530 :: 		RB5_bit = 0;                           //Establece el Max485 en modo de lectura;
	BCLR	RB5_bit, BitPos(RB5_bit+0)
;ADC_DAC.c,531 :: 		for (ipp=3;ipp<5;ipp++){
	MOV	#lo_addr(_ipp), W1
	MOV.B	#3, W0
	MOV.B	W0, [W1]
L_main87:
	...
	GOTO	L_main88
L__main170:
;ADC_DAC.c,532 :: 		Rspt[ipp]=0;;                      //Limpia la trama de respuesta
	MOV	#lo_addr(_ipp), W0
	ZE	[W0], W1
	MOV	#lo_addr(_Rspt), W0
	ADD	W0, W1, W1
...
;ADC_DAC.c,535 :: 		}
L_main81:
;ADC_DAC.c,538 :: 		DatoPtcn = 0;                             //Limpia la variable
	CLR	W0
	MOV	W0, _DatoPtcn
;ADC_DAC.c,539 :: 		for (ipp=0;ipp<Psize;ipp++){
	MOV	#lo_addr(_ipp), W1
	CLR	W0
	MOV.B	W0, [W1]
L_main90:
	...
	GOTO	L_main91
L__main171:
;ADC_DAC.c,540 :: 		Ptcn[ipp]=0;                          //Limpia la trama de peticion
	MOV	#lo_addr(_ipp), W0
	ZE	[W0], W1
	MOV	#lo_addr(_Ptcn), W0
	ADD	W0, W1, W1
...
;ADC_DAC.c,541 :: 		}
	GOTO	L_main90
L_main91:
;ADC_DAC.c,542 :: 		BanP = 0;                                 //Limpia la bandera de lectura de datos
	MOV	#lo_addr(_BanP), W1
	CLR	W0
	MOV.B	W0, [W1]
;ADC_DAC.c,544 :: 		}else{
...
;ADC_DAC.c,547 :: 		}
	GOTO	L_main94
L_main95:
;ADC_DAC.c,548 :: 		BanP = 0;                              //Limpia la bandera de lectura de datos
	MOV	#lo_addr(_BanP), W1
	CLR	W0
	MOV.B	W0, [W1]
;ADC_DAC.c,549 :: 		}
...
;ADC_DAC.c,550 :: 		}
L_main73:
;ADC_DAC.c,552 :: 		Delay_ms(50);                                   //Retraso necesario para que la Rpi tenga tiempo de recibir la trama de respuesta
	MOV	#11, W8
	MOV	#11309, W7
L_main97:
	DEC	W7
